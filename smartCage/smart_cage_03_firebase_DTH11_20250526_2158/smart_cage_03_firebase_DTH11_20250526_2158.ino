...
void setupWiFiAndFirebase() {
  ...
  // Read scan delay setting
  String scanDelayStr = readFirebase(ROOT + "/config/firebaseScanDelay");
  int scanDelayInt = scanDelayStr.toInt();
  if (scanDelayInt > 0) {
    firebaseDataBaseScanDelay = scanDelayInt * 1000;
  }
}
...
void setup() {
  ...
  // Assign tasks to different cores for load balancing
  xTaskCreatePinnedToCore(doorControlTask, "DoorTask", 4000, NULL, 1, NULL, 0);
  xTaskCreatePinnedToCore(servoFood1Task, "Food1Task", 4000, NULL, 1, NULL, 0);
  ...
  xTaskCreatePinnedToCore(mq2Task, "MQ2Task", 4000, NULL, 1, NULL, 0);
}
